...
3. **런타임 자원 분리** — 경로(`/agent/v3/*`)·그래프 인스턴스·Checkpointer·`thread_id` 네임스페이스(`v3_*`)·프론트 전역(`_AGENT_V3_*`)·DOM 컨테이너(`#agentV3Panel`)·영구 메모리 파일(`~/.uxermanager/agent_v3_memory.md`)을 v1·v2와 분리한다. v1 전역(`_AGENT_URL`·`_agentThreadId`)·v2 전역(`_AGENT_V2_*`)을 재사용·재정의하지 않는다.
...
| 2026-06-23 | HELP ▸ 소프트웨어 정보(About) 모달 + 수동 인앱 업데이트(자동 업데이트 비활성) | electron/main.js(setupAutoUpdater 수동화·IPC), electron/preload.js(IPC 브리지), index.html(Help 메뉴·About 모달), js/ui.js(About 상태머신·CMD_LIST·ESC), css/modal.css | 오너 요청(feature-dev 파이프라인). 앞서 넣은 자동 업데이트(시작 시 checkForUpdates·autoDownload·종료 시 설치·다운완료 dialog)를 **수동 전환**: autoDownload=false·autoInstallOnAppQuit=false, About 모달에서 IPC(updater:check/download/install + app:get-info)로만 트리거. updater:* 6이벤트를 렌더러로 forward해 다운로드/설치 **진행률 실시간 표시**. About는 앱명·버전(app.getVersion)·제작사·사이드카(/ping)·환경 표시. 웹/개발모드 graceful(영역 숨김·{ok:false,reason:dev}). preload onUpdaterEvent 해제함수 반환+closeAboutModal서 _updUnsub 호출(누수방지), setupAutoUpdater 1회 등록(중복방지), 동적값 textContent(XSS無). 검증: analyst→implementer→integration-checker(PASS·단축키/백업/상태/렌더 전부 N/A)→reviewer(PASS·경미4건 보강: 재오픈 초기화·error null·확인실패 표시·status null가드). node --check 3파일 통과. Agent v1/v2/v3 격리 미접촉. README §24·27·28 "자동"→"수동·인앱" 정정. 데스크탑 전용·재빌드 후 적용 |
| 2026-06-27 | v3 신규 툴 batch_update_names(공유) + v1.7.0 릴리스 | js/agent_tools.js(_agentToolBatchUpdateNames 신규·등록·라벨, 67종), README.md, docs/plan/클라이언트_툴_현황.html, electron/package.json(1.7.0) | 명시 매핑 목록(items)으로 여러 테이블·컬럼의 논리명/물리명을 한 번에 변경하는 write 툴(attrName 있으면 컬럼·없으면 테이블, 1회 호출=1회 승인, 항목별 성공/실패 반환). §9.1 의도적 공유 추가(agent_tools.js — v1·v2·v3 자동, v3 경로 무수정). vm 기능검증·node --check·run매칭(67) 통과. CI(v1.7.0 태그)로 정식 릴리스·자산3종 게시. 클라만 — 앱 새로고침이면 적용 |
| 2026-06-27 | v3 반복상한 16→35 + 질의 기반 승인 면제(오너 요청) | agent/v3/common/schemas.py(MAX_LOOP 16→35), agent/v3/nodes/react.py(auto_approve 게이팅), agent/v3/nodes/prep.py(_detect_auto_approve), agent/v3/common/state.py(auto_approve 필드), docs/plan/v3_react_hybrid.md | 오너 요청 — ① ReAct 반복 상한 MAX_LOOP 16→35 상향. ② 승인 게이트는 **기본 ON 유지**하되, 사용자가 질의에서 "승인 없이 진행해줘"·"묻지 말고 바로 실행"·"without approval" 등을 **자연어로 명시**하면 그 턴만 면제. (당초 needs_approval=False 전역 임시 비활성화로 넣었다가, 모든 사용자에게 안전 off 배포 위험 → 질의 기반 per-turn opt-out 으로 전환.) prep 이 last_user_text 를 정규식 감지→state['auto_approve']→react 가 쓰기/위험 판정 후 면제. 매 턴 재평가(전역 off 아님), 과탐 방지("승인 절차가 뭐야?" 미감지). v3 전용·격리 유지(화이트리스트 빔·상호참조 0). py_compile·그래프빌드·감지 양성7/음성5·MAX_LOOP=35 통과. **사이드카 재빌드 필요** |
| 2026-06-27 | V3 verify — 판정 전 확인용 read 툴 1회 호출(오너 요청) | agent/v3/common/schemas.py(V3VerifyProbe 신규), agent/v3/common/prompts.py(VERIFY_PROBE_SYSTEM), agent/v3/nodes/verify.py(async 전환·_run_probe), docs/plan/v3_react_hybrid.md | verify 가 V3Verdict 판정 전에 결과를 **직접 확인**하는 read 툴 1회 호출 단계 추가. LLM 이 need_check=true 면 proxy read 툴(목록 제시) 1개+인자를 골라 run_proxy_tool 로 서버 실행→_obs_text 요약을 관찰에 반영 후 판정. **읽기 전용만 허용**(catalog kind=="read", run_sql·쓰기/외부/위험 제외)이라 부수효과 0(승인 비활성화와 무관히 안전), 환각/쓰기 툴·호출 실패는 무시하고 판정 진행. verify_node async 전환(graph 는 이미 astream). 검증: py_compile·그래프 빌드·read필터(21툴·run_sql제외·run_select포함)·mock LLM E2E(run_select 확인 호출·관찰 누적·쓰기툴 차단) 통과. v3 전용·격리 유지(화이트리스트 빔·상호참조 0). **사이드카 재빌드 필요** |
| 2026-06-27 | V3 메모리 조회 최적화 — 조회는 answer 1회·verify 메모리 인지(오너 지적) | agent/v3/common/memory.py(is_memory_request=write전용), agent/v3/nodes/analyze.py(조회→answer 유도), agent/v3/nodes/verify.py([메모리]주입), agent/v3/common/prompts.py(VERIFY 메모리 pass 규칙) | 증상: "내 이름 뭐야?"가 act→ReAct 루프로 가고 verify 가 [메모리]를 못 봐 fail 2회 반복("5단계"). 수정: ① is_memory_request 를 **저장/삭제(write) 전용**으로 좁힘(조회는 강제 act 안 함) ② analyze 규칙 — 이미 기억된 정보 조회는 도구 불필요 → answer ③ verify 에 [메모리] 주입 + "답이 메모리/관찰에 있으면 pass(도구 관찰 없다고 미충족 금지)". 조회는 answer 1회로 직답, 혹 act 로 새도 verify 가 즉시 pass. 검증: write4/recall3 구분·py_compile·그래프빌드·격리 빔. **사이드카 재빌드 필요** |
| 2026-06-27 | V3 메모리 전 경로 참조 — answer·respond 노드도 [메모리] 주입(오너 요청) | agent/v3/nodes/answer.py(신규 격리복제), agent/v3/nodes/respond.py(신규 격리복제), agent/v3/graph.py(복제본 사용), docs/plan/v3_react_hybrid.md, proxy/python/README.md | 오너 요청 — 메모리를 react·analyze 뿐 아니라 **답변(answer) 모드·최종 응답(respond)** 에서도 참조. answer·respond 가 v1 공유라 §9.1 따라 v3로 복제해 시스템 프롬프트에 [메모리] 주입, 그래프가 복제본 사용. v1 answer/respond 무손상(diff 빔). load_memory() mtime 캐시로 "로드+변경 시 재로드" 충족. 검증: py_compile·그래프빌드·복제본 출처(__module__)·메모리 주입 확인·격리(화이트리스트 빔). **사이드카 재빌드 필요** |
| 2026-06-27 | V3 메모리 라우팅 버그수정 — '기억해' 가 answer 로 새던 문제 | agent/v3/common/memory.py(is_memory_request 정규식), agent/v3/nodes/analyze.py(메모리 의도 시 answer→act 강제), docs/plan/v3_react_hybrid.md | 증상: "내 이름 기억해둬"가 analyze 에서 answer 로 분류돼 react 루프(remember 툴 위치)에 도달 못 함 → 모델이 "remember 툴 없다"·"기억하겠습니다"만 함(저장 안 됨). 원인: ANALYZE_SYSTEM(v1 공유, 수정 불가)이 메모리 저장을 도구 필요 작업으로 안 봄 + 약한 로컬 LLM 이 프롬프트 힌트 무시. 수정: is_memory_request 정규식(기억/외워/잊어/조회 등)으로 **결정적 감지** → analyze 가 answer 로 분류해도 메모리 의도면 route=act·intent.kind=act 강제(없으면 remember goal 합성). v1 ANALYZE_SYSTEM 무수정(v3 복제본에만 보정 — 격리). 검증: 카탈로그 등록 확인·is_memory 양성4/음성2·py_compile. **사이드카 재빌드 필요**(설치본은 재릴리즈) |
| 2026-06-27 | V3 영구 메모리 기능 — remember/recall/forget(오너 요청) | agent/v3/common/memory.py(신규), agent/v3/nodes/memory_exec.py(신규), agent/v3/common/schemas.py(MEMORY_TOOL_CATALOG), agent/v3/common/prompts.py(REACT 메모리 규칙), agent/v3/nodes/react.py(카탈로그·[메모리]주입·라우팅·가드⑤), agent/v3/nodes/analyze.py([메모리]주입), agent/v3/graph.py(memory_exec 노드), docs/plan/v3_react_hybrid.md, proxy/python/README.md | 사용자가 자연어로 영구 기억 지시("이건 기억해"·"앞으로 항상 ~해")하면 md 파일에 저장하고 에이전트 실행마다 시스템 프롬프트 [메모리]에 로드·참조. 저장소 ~/.uxermanager/agent_v3_memory.md(공유 DATA_DIR 경로만 재사용), mtime 캐시로 파일 변경 시 자동 재로드. 메모리 툴 3종(location="memory", 승인 면제 — react_route 가 approve 전 분기) + memory_exec 노드. react·analyze 프롬프트에 [메모리] 주입, REACT_SYSTEM 에 "항상 존중·영구의도만 remember·단발성 미기억" 규칙, react 가드⑤(remember 2회 초과 finish). 검증: py_compile·그래프빌드·메모리모듈(append/dup/items/render/mtime재로드/forget)·라우팅·memory_exec E2E 통과. v3 전용·격리 유지(화이트리스트 빔·상호참조 0). **사이드카 재빌드 필요** |
| 2026-06-28 | 스플래시 2모드(경량 재현 + 원본 홍보영상) 무작위 표시 | splash/promo/(promo.html·animations.jsx/.js·promo-scenes.jsx/.js·README 신규), vendor/react·react-dom.production.min.js(신규), index.html(head 가드에 모드 무작위 + promo iframe 마크업·공유 컨트롤 class화), css/splash.css(모드 전환·promo 컨트롤바), js/splash.js(모드 분기·iframe 주입·promo→lite 폴백), electron/package.json(extraFiles 에 splash/ 추가), README §26 | 오너 요청 — 경량 재현과 원본 홍보영상을 둘 다 적용하고 시작 시 무작위로 하나 표시. 원본 promo(claude_design 'AgenticERM 홍보영상.dc.html', React/JSX 30초)를 데스크탑서 오프라인·즉시 재생하도록 **사전 트랜스파일**: animations.jsx·promo-scenes.jsx 를 @babel/standalone 으로 JSX→createElement 변환 후 **IIFE 격리**(두 파일 top-level const/function 이 전역 렉시컬 스코프서 충돌 'Stage already declared' → IIFE 로 해소, 파일 간 통신은 window 전역으로만), promo.html 이 로컬 React UMD(벤더링, CDN/Babel 런타임 0)로 PromoVideo 마운트·재생바 숨김. head 인라인 가드가 Math.random()<0.5 로 splash-mode-{promo|lite} 클래스 선택(첫 페인트 무플래시), splash.js 가 분기. 공유 컨트롤(상태·시작하기·다시표시안함)을 class(js-sp-*)화해 두 모드 공용 — 데이터 로드 완료 시 양쪽 활성. promo 7초 내 ready 없으면(스크립트 실패 등) lite 자동 폴백. extraFiles 에 splash/ 추가(미추가 시 설치본서 promo 404). React 는 promo iframe 안에서만 — 메인 앱 무영향. 검증: 트랜스파일·node --check·node 통합 실행(PromoVideo=function)·headless Chrome로 promo 풀렌더(앱셸·ERD·관계·캡션) + 양모드 스플래시 스크린샷·컨트롤 활성 확인. 클라+빌드설정 — 앱 새로고침/재빌드 시 적용 |
| 2026-06-28 | 시작 화면(스플래시) 추가 — 홍보영상 브랜드 경량 재현 | js/splash.js·css/splash.css(신규), index.html(head 가드·오버레이 마크업·splash.js 로드·About 토글 행), js/ui.js(openAboutModal 동기화·onAboutSplashToggle), js/main.js(splashMarkDataLoaded 훅), README.md §26 | 오너 요청 — claude_design MCP 의 'AgenticERM 홍보영상.dc.html'(React/Babel 기반 30초 promo)을 스플래시로. promo 런타임(support.js+React+@babel/standalone ~3MB, unpkg CDN)은 데스크탑 스플래시에 부적합 → 오너 선택 '경량 재현'으로 promo 브랜드(엔티티 노드 로고·팔레트·회전 캡션 5종·'112 컬럼 표준화' 카운터 루프)를 의존성 0 vanilla HTML/CSS/JS 로 자체 구현. 첫 페인트 즉시 표시(head 인라인 가드로 비활성 시 깜빡임 방지), **모든 데이터 로드 완료 시 '시작하기'(닫기) 활성**(데스크탑=loadWorkspacePC.finally, 웹=동기로드 직후 splashMarkDataLoaded; 12초 안전장치). 영구 비활성: '다음에 표시 안 함' 체크 + 소프트웨어 정보(About) 모달 토글(localStorage aerm_splash_disabled). 검증: node --check 3파일·headless Chrome 렌더 스크린샷(로고·카운터·준비완료·버튼활성 확인). 에이전트(v1/v2/v3)·격리 미접촉. 클라만 — 앱 새로고침이면 적용 |
| 2026-06-28 | V3 버그수정 — 질의 중단 시 대화 세션(thread_id) 유실 | js/agent_v3/client_v3.js(catch 의 thread_id 초기화 분기) | 증상(스크린샷 042309): 직전 턴에 에이전트가 86개 컬럼 표를 보여줬는데, 다음 턴 "리스트업된 컬럼 한글적용"에 "표/목록이 확인되지 않는다"며 컨텍스트를 못 봄. 원인: agentV3Send catch 가 **모든 예외에서 _agentV3ThreadId=null** 로 초기화 — 사용자가 중단(■) 버튼을 누르면 AbortError 가 나는데 이때도 thread_id 가 비워져 **다음 질의가 새 스레드로 시작**(직전 대화·표 전부 유실). 사용자는 중단 후 재질의했으므로 컨텍스트 단절. 수정: AbortError(사용자 중단)는 **thread_id 보존**(대화 세션 유지), TypeError(프록시 미연결)·HTTP/백엔드 오류만 초기화(스레드 상태 불확실). 백엔드 checkpointer·HISTORY_WINDOW(8)는 정상 — 프론트 초기화가 단독 원인. v3 전용·격리 빔·node --check 통과. 클라만 — **앱 새로고침이면 적용** |
| 2026-06-28 | V3 버그수정 — verify 미충족인데 respond가 성공 환각 + localize 우회 방지 | agent/v3/nodes/respond.py(verdict 정직 힌트), agent/v3/common/prompts.py(REACT 한글화 규칙 강화) | 증상(스크린샷 035316): "컬럼 논리명 한글화"에서 모델이 localize_attribute_names 를 건너뛰고 get_statistics 만 반복(실제 변경 툴 미호출) → verify 가 fail(보완)·미충족(g1) 2회 후 MAX_VERIFY 종료했는데, **최종 respond 가 "모든 컬럼 한글화 완료(미완료 항목 없음)"로 성공을 지어냄**. 원인: respond_node 가 past_steps·replan_route 만 보고 **verdict 를 안 봄** → 미충족 종료를 모른 채 성공 서술. 수정: ① respond.py — verdict.fulfilled False 또는 adherence in(fail,partial)이면 "[검증 결과] 완전히 충족 안 됨·실행된 툴 결과 없으면 미수행·완료/성공 단정 금지·실제 수행분과 미충족분 구분 보고" 힌트 주입(v3 격리 복제본이라 v1 respond 무손상) ② REACT_SYSTEM — '자체 번역하겠다'며 localize 건너뛰기 금지, get_statistics/describe_table 반복만 하고 변경툴 미호출은 미수행, 실제 변경툴이 관찰에 성공으로 남기 전 finish 금지 명시. 검증: respond verdict 힌트 3케이스(미충족 주입/통과·무verdict 미주입) 통과·py 구문·격리 빔. **사이드카 재빌드 완료(04:01)** |
| 2026-06-28 | 신규 툴 localize_attribute_names — 컬럼 논리명 한글화(공유) + batch_update_names 빈 items 가드·v3 가이드 | js/agent_tools.js(_agentToolLocalizeAttributeNames·_agentFetchDbComments 신규·등록·라벨, batch_update_names 빈 items 에러 강화, 68종), agent/v3/common/prompts.py(REACT 한글화 규칙), README.md, docs/plan/클라이언트_툴_현황.html | 증상: "컬럼의 논리명도 한글로 변경해줘"에 react 가 batch_update_names 를 골랐으나 items(112개) 빈 배열로 실패. 원인: batch_update_names 는 LLM 이 {entityId, attrName, logicalName} 를 손수 채워야 하는데, 약한 모델이 한글명 112개를 args_json 에 직렬화하지 못해 빈 items. 해법(오너 결정 "셋 다 우선순위"): standardize_attribute_names 의 **역방향** 결정적 배치 툴 localize_attribute_names 신규 — 컬럼 물리명을 ① 표준용어사전 역변환 ② 실패 시 DB 컬럼 코멘트(/schema/comments) 순으로 1회 호출=1회 승인 일괄 채움, 못 채운 소수만 unresolved 로 보고→batch_update_names 로 보완. onlyEmpty 기본 true(이미 한글 보존). **사전 역변환은 2단(_agentStdIndexMap)** — 전체 물리명이 term(/stddict/index?table=term) 표준어면 그 한글 정확일치, 아니면 _ 로 나눠 word(table=word) 원자단어 abbr 를 모두 해소 시 합성(부분 실패는 미해소). [실데이터 검증: term 15368·word 4284 로 EMP_NO→직원번호·DEPT_CD→부서코드·MM10_SND_PLAN_AMT→10월수급계획금액 정확일치·미해소 보존] + batch_update_names 빈 items 에러를 "localize 권장·describe_table 선행" 안내로 강화 + REACT_SYSTEM 한글화 규칙(items 비우지 말 것). §9.1 의도적 공유(agent_tools.js — v1·v2·v3 자동), REACT 규칙은 v3 전용. 검증: 라이브 사이드카 역변환 6/6·mock 코멘트폴백 8/8·node --check·py 구문·격리 빔. **사이드카 재빌드 완료(1.11.0, REACT 가이드 반영)**, 툴 자체는 앱 새로고침이면 적용. /schema/comments·stddict 는 기존 엔드포인트(무변경) |
| 2026-06-28 | 툴 desc 범위 혼동 일괄 점검·명확화(공유) | js/agent_tools.js(find_tables·describe_table·list_relations·generate_markdown·generate_table_spec·generate_data_dictionary·generate_erd_report·batch_rename_attributes·update_attribute·update_entity·normalize_check·suggest_normalization desc) | batch_update_entities 건에 이어 카탈로그 전수 점검(67툴). react 가 desc만 보므로(detail 미노출) 혼동 축별로 desc 자기설명화: ① ERD(캔버스) vs 연결 DB 실물 — find_tables·describe_table·list_relations 에 "ERD(캔버스)" 명시 + 각각 list_db_tables/find_db_column·describe_db_table·get_db_constraints 교차참조(describe_table↔describe_db_table 이름 거의 동일해 최대 혼동축) ② 산출물 4종 범위 분리 — generate_markdown(채팅 텍스트)·table_spec(테이블별 컬럼 표)·data_dictionary(전 테이블 컬럼 일람)·erd_report(개요+관계+통계 종합) ③ 이름변경 패밀리 — batch_rename_attributes(패턴/컨벤션)↔batch_update_names(개별 지정), update_attribute/update_entity(단건)↔batch_update_names(일괄) 교차참조 ④ 정규화 — normalize_check(빠른 목록)↔suggest_normalization(상세 수정안). §9.1 의도적 공유(v1·v2·v3 자동, 카운트 67 불변). node --check 통과. 클라만 — **앱 새로고침이면 적용** |
| 2026-06-28 | 버그수정 — batch_update_entities/batch_update_names 범위 혼동(desc 명확화, 공유) | js/agent_tools.js(두 툴 desc·detail) | 증상: "엔티티의 논리명 변경" 요청에 에이전트가 batch_update_names 대신 batch_update_entities 를 선택. 원인: react 는 카탈로그 desc만 보는데(detail 미노출) batch_update_entities desc 가 "여러 테이블 **속성** 일괄 수정"이라 이름까지 포함하는 것으로 오인(실제 범위는 설명·색상·예상행수뿐). 수정: batch_update_entities desc 를 "설명·색상·예상행수만 일괄 수정(이름은 못 바꿈 → 논리명/물리명은 batch_update_names)"로 범위 명시·교차참조, batch_update_names desc 를 "여러 테이블·컬럼의 논리명/물리명 항목별 일괄 변경 — 이름 바꾸기는 이 툴"로 강화. detail 도 동기화. §9.1 의도적 공유(agent_tools.js — v1·v2·v3 자동, 카운트 불변). node --check 통과. 클라만 — **앱 새로고침이면 적용** |
| 2026-06-28 | V3 처리 단계 가독성 — 누적 반복 제거 + 타자기 스트리밍(오너 요청) | js/agent_v3/observe_v3.js | 증상: 약한 로컬 LLM 이 매 ReAct 스텝의 thought 에 직전 맥락(요청·현황)을 통째로 되풀이 → 처리 단계 추적이 같은 문구를 누적 반복해 가독성 저하(스크린샷 2026-06-28 024409). 수정(프론트 단독): ① 턴 단위 '본 문장' 집합(trace body 에 부착)을 두고 문장 단위로 정규화 비교 → 이전 단계와 겹치는 문구는 빼고 **이번 단계의 새 계획만** 렌더(action 라벨은 항상 표시해 스텝 식별 유지), 의도(🎯) 요약 문장도 집합에 선등록해 반복 차단 ② 새 문구를 textContent 타자기 효과로 **스트리밍 출력**(_agentV3StreamInto, XSS 안전·스텝 동시 진행 무충돌). 백엔드·프롬프트 무변경(모델 행동과 무관히 결정적). v3 전용 DOM·격리 유지(화이트리스트 빔·상호참조 0), node --check 통과. 클라만 — **앱 새로고침이면 적용** |
| 2026-06-29 | 신규 기능 — DB 저장·공유 다이어그램(M1~M6, feature/db-backed-shared-diagrams) | proxy/python·nodejs(어댑터4×2 멀티풀+execute_params·connector·config·schema·execute·신규 erd_store 라우터·main/index 등록), js/erd_db_store.js(신규)·state.js·diagrams.js·explorer.js·ui.js·index.html·css/panel.css, proxy/python/routers/schema.py·agent/tools_proxy.py(메타필터), docs/plan/db_backed_shared_diagrams.md, README 3종 | **DB를 다이어그램 공유 동기화 백엔드로** 사용(CRDT 중앙서버 대안 — DB가 상시·다중접속·영속 인프라 대체). 다이어그램별 독립 DB 연결(`source:'local'\|'db'`·`connection.profileName`), 정의를 그 DB의 **격리 메타테이블 `UXER_ERD_DIAGRAM`**(업무테이블 무참조)에 저장, 접속정보 공유=다이어그램 공유. **M1** 프록시 단일활성→요청별 profileName+어댑터 멀티풀(하위호환 active 폴백, 기존 execute/load_config 무변경) **M2** /erd-store 5엔드포인트·4DB 멱등DDL·**드라이버 네이티브 파라미터 바인딩**(문자열 이스케이프 폐기로 MySQL 백슬래시 결함 회피)·409 낙관적잠금 **M3** 생성UI 로컬/DB·erd_db_store.js·Explorer 배지·백업 자동포함 **M4** version 낙관적잠금·8s폴링(중첩가드·저장중스킵)·에코루프가드(`_erdDbApplying`)·undo stale version 보정 **M5** 리버스 엔지니어링 초기채움(reverse_engineer.js 빌더 재사용)+포워드 엔지니어링 진입(forward_engineer.js 재사용+프로파일 불일치 경고) **M6** 자격증명 공유 경고·메타테이블 introspection 숨김(schema.py·tools_proxy.py `_filter_meta`). 데스크탑(Electron) 전용. **에이전트 v1/v2/v3 격리 유지** — agent/ 변경은 tools_proxy.py 메타필터(의도적 공유·동작 보존)뿐, 그래프/노드/v2/v3/js·agent_*·forward/reverse_engineer.js 코어 무변경. feature-dev 3패스(analyst→implementer→integration-checker→reviewer) 통과, 심각 3건(409 응답 비대칭·undo stale version·FE 경고 API 오류) 리뷰서 수정. py_compile·node --check 전체 통과. **사이드카 재빌드 + 앱 새로고침 필요**. 인앱 E2E·profileName 인지 FE·멀티풀 LRU 상한은 후속/사용자 검증 |
| 2026-06-29 | 기능 — 엔티티 속성 행 마우스 오버 하이라이트 | js/canvas.js(hoveredAttr 상태·drawEntity 행 하이라이트·mousemove hit-test·hover 리셋) | ERD는 canvas 렌더라 CSS hover 불가 → 마우스 위치를 속성 행에 hit-test 하는 JS 구현. mousemove 에서 hitEnt 의 펼쳐진 본문 영역 내 행 인덱스(`floor((w.y-(y+HEADER_H))/ROW_H)`, 0≤idx<attrs.length·collapsed 제외)를 계산해 hoveredAttr={entity,index} 설정, 변경 시에만 render(). drawEntity 의 attrs 루프에서 해당 행에 반투명 액센트(`hexAlpha(COLOR.lineHover,0.16)`)+좌측 2.5px 액센트 바를 pk/fk 배경 위에 겹쳐 칠함(텍스트는 위에 유지·테마 색상 자동 추종). mouseup/리셋 시 hoveredAttr 정리(기존 hoveredEntity 와 동일 수명). node --check 통과. 클라만 — 앱 새로고침이면 적용. 에이전트 v1/v2/v3 격리 미접촉 |
| 2026-06-29 | 기능 — 관계선 굵기 2배 + v1.14.3 릴리스 | js/canvas.js(관계선 stroke lineWidth) | 오너 요청 — 관계선 두께를 현재의 2배로. 곡선 모드(활성/기본 `2/1.5`→`4/3`), 직각 세그먼트 모드(호버/활성/연결/기본 `2.5/2/2/1.5`→`5/4/4/3`), 연결 강조 글로우(`6`→`12`, 선 두께 비례 유지)를 모두 2배. 크로우풋·1/N 카디널리티 끝점 마커는 "선 굵기" 아님이라 유지. node --check 통과. 클라만 — 앱 새로고침이면 적용. 에이전트 격리 미접촉. 직전 속성 hover 하이라이트와 함께 CI(v1.14.3 태그) 릴리스 |
| 2026-06-29 | 버그수정 — DB 다이어그램 Explorer ⬆(포워드 엔지니어링) 버튼 무반응 + v1.14.2 릴리스 | js/explorer.js, js/diagrams.js, electron/package.json(1.14.2) | 위 M5에서 추가된 Explorer 패널의 DB 다이어그램 ⬆ 버튼 onclick 이 `JSON.stringify(d.id)`(큰따옴표)를 큰따옴표로 묶인 onclick 속성 안에 삽입 → innerHTML 파싱 시 `x.id===` 직후 `"`에서 속성이 조기 종료되어 깨진 JS 조각만 남아 클릭 무반응. 수정: onclick 을 다른 버튼과 동일하게 `_openFEForDbDiagram('${d.id}')`(작은따옴표 id) 로 변경 + `_openFEForDbDiagram(diagOrId)` 가 id 문자열·다이어그램 객체 모두 받도록 보강(인라인 핸들러의 `let diagrams` 전역 lexical 의존 제거 — 함수 내부 조회로 일원화). 메뉴바·엔티티 우클릭 진입은 정상이었음. node --check 2파일 통과. 클라만 — 앱 새로고침이면 적용. 에이전트 v1/v2/v3 격리 미접촉. CI(v1.14.2 태그)로 릴리스 |
| 2026-07-27 | 리버스·포워드 엔지니어링 Supabase 지원(dbType 추가) | proxy/python(db/adapters/supabase.py 신규·postgres.py 연결옵션·connector.py sql_dialect·routers/config·schema·erd_store·agent/tools_proxy·db_docs), proxy/nodejs(동일 구조 병행), js/profile_manager.js·forward_engineer.js·agent_tools.js, README 3종, docs/plan/클라이언트_툴_현황.html | Supabase 는 PostgreSQL 이라 SQL 방언·introspection·DDL 은 그대로 쓰되 **연결 조건만 다르다** → dbType `supabase` 를 1급 유형으로 추가하고 `sql_dialect(supabase→postgres)` 로 방언만 정규화(어댑터 조회는 원본 유형 유지 — 연결옵션이 달라짐). supabase 어댑터가 ssl=require + statement_cache_size=0(트랜잭션 풀러 6543 PgBouncer 는 prepared statement 미지원) + 기본값(5432·postgres)을 주입해 postgres 어댑터에 위임하고, IPv6 전용 직접연결·풀러 사용자명(postgres.<ref>) 등 실패 원인을 오류에 덧붙인다. 풀 키에 dbType 추가(연결옵션 다른 풀 분리). 프론트: 프로파일 관리에 'Supabase (PostgreSQL)' 종류 + 대시보드 연결문자열 붙여넣기 파서(URL 디코딩·[YOUR-PASSWORD] 자리표시자 무시·적용 후 입력칸 즉시 비움), 포워드 dialect 자동감지 supabase→postgresql. 부수 수정: agent generate_ddl 이 'postgres'/'supabase' 를 buildDDL 방언 'postgresql' 로 정규화(기존엔 'postgres' 가 방언표에 없어 폴백되던 잠재버그). §9.1 의도적 공유 추가(tools_proxy·db_docs·agent_tools.js — v1·v2·v3 자동 적용, 기존 유형 동작 불변; agent/v2·v3 경로 무수정·격리 diff 빔). 검증: 어댑터/방언/풀키/힌트 단위검증, 목 어댑터 FastAPI E2E(리버스 GET /schema → 테이블·FK·PG 방언 확인, 포워드 GET /schema/tables + POST /execute/stream 2건 성공), 연결문자열 파서 8케이스, generate_ddl 방언 9케이스, py_compile·node --check 전부 통과. **사이드카 재빌드 필요**(+앱 새로고침). 실제 Supabase 인스턴스 E2E = 사용자 검증 |
