...
| `js/agent_tools.js` | Agent 클라이언트 툴 **57종**(엔티티·관계·속성 CRUD, 선택·뷰·하이라이트, 일괄·관계자동화, 분석·검증·통계, 섹션·메모, 다이어그램·스냅샷, 테이블정의서·데이터사전·ERD명세서 산출물 등) · 드래프트 커밋·원자적 undo · 표준용어사전 연동 · async 툴 지원. 서버 측 DB 툴은 `agent/tools_proxy.py`(20종, introspection·데이터분석·포워드엔지니어링). v1·v2·v3 공유 |
