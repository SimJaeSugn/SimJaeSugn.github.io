...
| `memory` | Python 서버 (로컬 md 파일 R/W) | **remember·recall·forget** (v3 신규) | **면제**(로컬 파일·저위험) |
...
- `remember`/`recall`/`forget`(location="memory")는 영구 메모리 툴이다. `memory_exec` 노드가
  `~/.uxermanager/agent_v3_memory.md`에 항목을 저장/조회/삭제한다. 저장된 메모리는 매 턴 `load_memory()`
  (mtime 캐시 — 파일 변경 시 자동 재로드)로 react·analyze 시스템 프롬프트의 `[메모리]` 섹션에 주입된다.
  `react_route`가 approve 보다 먼저 `memory`로 분기해 승인을 면제한다.
...
- **준수 검증 verify(2026-06-06)**: `react`의 `finish`가 곧장 종료하지 않고 `verify` 노드를 거친다 — `[분석된 의도]`의 goal 이 `[관찰 기록]`으로 충족됐는지 `V3Verdict`(adherence pass/partial/fail, missing, next)로 구조 판정. 충족(pass)/판정불가 → respond, 보완 가능(partial+continue) → 미충족 내용을 관찰에 남기고 `react`로 되돌림(무한 검증은 `MAX_VERIFY=2`로 차단). 모델의 성급한 finish·목표 누락을 잡는다. SSE `verdict` 이벤트로 관측 노출(observe_v3 렌더). **이로써 M3 완료** — verify·관찰 기반 종료조건·승인 모델 모두 구현.
  ...
  - **respond 의 verdict 반영(2026-06-28, 버그수정)**: verify 가 미충족(`fulfilled=False` 또는 adherence `fail`/`partial`)으로 `respond`에 도달했는데 **respond 가 verdict 를 안 봐서 성공을 지어내던** 버그(스크린샷 035316 — 변경 툴 미호출로 verify 가 fail 2회 후 MAX_VERIFY 종료했으나 최종 보고는 "한글화 완료(미완료 없음)"). 수정: `respond.py`(v3 복제본)가 `state['verdict']`를 읽어, 미충족이면 시스템 힌트로 "**완료/성공 단정 금지** · [수행 결과]에 실행된 툴 결과 없으면 미수행 · 수행분과 미충족분을 사실대로 구분 · 다음 단계 안내"를 주입한다. 통과(pass)·목표 없음·verdict 없음이면 힌트 미주입(정상 보고). v1 respond 무손상(격리 복제본만 수정). 단위검증 3케이스 통과. 함께 `REACT_SYSTEM`에 "한글화는 `localize_attribute_names`를 먼저 호출, '자체 번역하겠다'며 건너뛰기 금지, 실제 변경 툴이 관찰에 성공으로 남기 전 finish 금지" 규칙 강화(`get_statistics`만 반복하고 변경 툴 미호출 → 미수행).
...
- **영구 메모리 기능 — remember/recall/forget(2026-06-27, 오너 요청)**: 사용자가 자연어로 영구 기억을 지시("이건 기억해"·"앞으로 항상 ~해"·"잊지 마")하면 그 내용을 md 파일에 저장하고, 에이전트 실행마다 시스템 프롬프트에 로드해 참조한다. **저장소**: `~/.uxermanager/agent_v3_memory.md`(공유 `DATA_DIR` 경로 상수만 재사용). **모듈**: `agent/v3/common/memory.py` — `load_memory()`(mtime 캐시 → 파일이 툴/사용자 편집으로 바뀌면 자동 재로드), `append_memory`(중복 스킵), `forget_memory`(match/all), `render_memory_section`. **툴**(location="memory", 승인 면제): `remember`(저장)·`recall`(조회)·`forget`(삭제) — `MEMORY_TOOL_CATALOG`(schemas.py). **노드**: `memory_exec`(nodes/memory_exec.py)가 실행, `react_route`가 approve보다 먼저 `memory`로 분기. **주입(전 경로)**: `analyze`·`answer`·`react`·`respond` 모든 응답 생성 노드의 시스템 프롬프트에 `[메모리]` 섹션 추가 — 답변(answer) 모드든 ReAct 모드든 항상 참조(2026-06-27 보강). answer·respond 는 v1 공유라 §9.1 따라 v3로 복제(`agent/v3/nodes/answer.py`·`respond.py`, v1 무손상)해 주입. `REACT_SYSTEM`에 "메모리 항목 항상 존중·영구 의도일 때만 remember·단발성 지시는 기억 안 함·메모리 툴엔 describe_tool 금지" 규칙. **로드/재로드**: `load_memory()` mtime 캐시 — 턴마다 호출하되 파일이 바뀔 때만 다시 읽는다(에이전트 실행 시 로드+변경 시 재로드). **가드 ⑤**: 한 턴 remember 2회 초과 시 finish. 검증: py_compile·그래프 빌드·메모리 모듈(append/dup/items/render/mtime 재로드/forget match·all)·라우팅(remember/recall/forget→memory)·memory_exec(저장·past_steps·recall) 통과. v3 전용·격리 유지(화이트리스트 빔·상호참조 0). **사이드카 재빌드 필요**.
