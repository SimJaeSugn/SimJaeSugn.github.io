...
async function _agentExecTools(calls, bubble) {
  ...
  for (const c of (calls || [])) {
    ...
    if (!def) r = { id: c.id, ok: false, error: '알 수 없는 툴: ' + c.tool };
      ...
      try { r = { id: c.id, ...(await def.run(_agentDraft, c.args || {}, _agentIdRemap)) }; }  // async 툴(표준사전 등) 지원
...
async function agentSend() {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      ...
      bubble.innerHTML = '⚠ 프록시(127.0.0.1:3737)에 연결할 수 없습니다.<br>AgenticERM 데스크탑 앱 또는 프록시를 실행하세요.';
    } else {
