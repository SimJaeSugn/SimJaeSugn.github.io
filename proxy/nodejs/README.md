...
> Oracle Instant Client 다운로드 (Basic 패키지): https://www.oracle.com/database/technologies/instant-client/downloads.html  
> 시스템 PATH 등록 (관리자 CMD): `setx PATH "%PATH%;C:\instantclient_23_0" /M` → 미들웨어 재시작  
> `clientLibDir` 필드는 참조용으로 저장되며, Thick 모드 활성화는 시스템 PATH 등록을 통해서만 동작한다.
...
```json
{
  ...
  "password":     "secret",
  "schema":       "public",
  "clientLibDir": "C:\\oracle\\instantclient_21_3"
}
```

> `schema` 는 선택 필드. PostgreSQL 의 경우 리버스 엔지니어링·스키마 조회 대상 스키마를 지정한다.
> 미지정 시 `public` 사용. MySQL/MSSQL/Oracle 은 사용하지 않는다.
> 허용 문자: 알파뉴메릭, 언더스코어, 점만 가능.
>
> `clientLibDir` 는 선택 필드 (Oracle 전용). 참조용 경로 저장 — Thick 모드 활성화는 시스템 PATH 등록을 통해서만 동작한다.
> Oracle 이외의 dbType에서는 무시된다.
...
> `schema` 는 선택 필드 (PostgreSQL 전용).
> `clientLibDir` 는 선택 필드 (Oracle 전용). 참조용 경로 저장 (Thick 모드는 시스템 PATH로 활성화).
...
`schema` 미지정 시 기존 값 유지. `clientLibDir` 미지정 시 기존 값 유지 (Oracle 전용 선택 필드).
...
저장된 프로파일의 비밀번호를 복호화해 평문으로 반환(확인용). 레거시 키로 암호화된 경우 현재 키로 재암호화 마이그레이션도 수행한다. 비밀번호가 없으면 빈 문자열을 반환한다.

**응답**
```json
{ "ok": true, "password": "s3cr3t" }
```
...
`columns[].isUnique` 는 UNIQUE 제약 또는 UNIQUE 인덱스가 걸린 컬럼이면 `true`.
`columns[].isAutoIncrement` 는 PG 의 `nextval(...)`, MySQL `auto_increment`, MSSQL `IDENTITY`, Oracle `IDENTITY` 컬럼이면 `true`.
`fks[].card` 는 from 컬럼이 UNIQUE 또는 PK 단독이면 `"1:1"`, 그 외 `"1:N"`.
