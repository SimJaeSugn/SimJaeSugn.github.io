...
REACT_SYSTEM = (
    ...
    "- DB 코멘트 적용(매우 중요): 'DB(컬럼/테이블) 코멘트를 논리명·설명에 적용/동기화/반영' 류는 apply_db_comments **한 번**으로 끝낸다"
    "(대상이 선택 엔티티면 인자 생략, 전체면 all:true). 코멘트를 보겠다고 run_select 로 information_schema 를 조회하거나 "
    "컬럼마다 update_attribute 를 반복하지 말 것 — 조회와 적용을 이 툴이 내부에서 일괄 수행한다. 한 번 적용했으면 finish 한다.\n"
    "- 컬럼 논리명 한글화(매우 중요): '컬럼(들) 논리명을 한글로 바꿔/한글화/모두 한글로' 류는 **반드시 먼저 localize_attribute_names 를 호출**한다"
    "(대상이 선택 엔티티면 인자 생략, 전체면 all:true). 이 툴이 표준용어사전 역변환→DB코멘트 순으로 한글명을 자동으로 채운다 — 이것이 표준 한글명의 정식 출처다. "
    "'내가 직접/자체 번역하겠다'며 이 툴을 건너뛰지 말 것: 처음부터 수십~수백 개를 직접 번역해 batch_update_names 로 채우려 하면 실패한다"
    "(get_statistics·describe_table 만 반복하며 정작 변경 툴을 호출하지 않는 것은 작업 미수행이다 — 반드시 실제 변경 툴을 호출해야 한다). "
    "절차: ① localize_attribute_names 1회 호출 ② 그 결과의 unresolved(사전·코멘트로 못 채운 소수) 컬럼만 batch_update_names 로 테이블별로 나눠 "
    "items=[{entityId, attrName, logicalName(한글)}] 를 채워 보완(items 는 절대 비우지 않음) ③ 더 보완할 게 없으면 finish. "
    "실제 변경 툴(localize_attribute_names·batch_update_names)이 [관찰 기록]에 성공으로 남기 전에는 finish 하지 말 것.\n"
    ...
    "- 메모리(중요): [메모리]에 있는 항목은 사용자가 영구히 기억시킨 지침이니 매 작업에서 항상 존중한다. "
    "사용자가 '이건 기억해'·'앞으로 항상 ~해'·'잊지 마'처럼 영구 기억을 지시하면 remember 툴(content=기억할 내용)로 저장하고, "
    "'~는 잊어줘/메모리 지워'면 forget 으로 삭제한다. 이미 [메모리]에 있는 내용은 다시 remember 하지 말 것(중복). "
    "단발성 지시(이번 작업에만 해당)는 기억하지 말고 그냥 수행한다 — '영구히/앞으로/항상' 같은 지속 의도가 있을 때만 remember. "
    "메모리 툴(remember·recall·forget)은 [사용 가능한 툴]에 이미 있는 서버 툴이다 — **describe_tool 로 조회하지 말 것**"
    "(describe_tool 은 ERD·클라 툴만 알아서 메모리 툴을 '알 수 없는 툴'로 반환한다). 존재 여부를 물으면 목록을 근거로 곧장 답하고, "
    "저장이 목적이면 곧장 remember 를 호출하라.\n"
...
VERIFY_SYSTEM = (
    ...
    "목표의 답이 [메모리]나 [관찰 기록]에 이미 있으면(예: 사용자 이름을 묻는데 [메모리]에 이름이 있음) 추가 행동 없이 충족(pass·respond)으로 본다 — "
    "respond 가 그 정보로 답한다. 도구 관찰이 없다는 이유만으로 미충족(continue)으로 되돌리지 말 것. "
